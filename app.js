...
import { FamilyLife } from './family-life.js?v=1';
...
let devices,family;const familyElements=new Map();let familyUiTime=0;
...
function initRenderer(){
 ...
 scene=new THREE.Scene();scene.add(family.root);scene.background=new THREE.Color('#e9eee3');camera=new THREE.PerspectiveCamera(42,1,.03,250);ortho=new THREE.OrthographicCamera(-15,15,10,-10,.03,250);activeCamera=camera;
...
async function buildFloor(floor){
 ...
 devices.buildLights(floor,group);if(floor==='F2')addLandingConnector(group,overviewDefinition);floorCache.set(floor,group);family.addFloor(floor);buildFamilyControls();return group;
}
...
function updateFamilyUi(){
 ...
 for(const p of family.people){const e=familyElements.get(p.id);if(!e)continue;if(refresh)e.status.textContent=(family.paused?'已暂停 · ':!family.shown?'已隐藏 · ':'')+floorInfo[p.floor].name+' · '+family.status(p);const bubble=e.bubble,point=p.position.clone().add(vec([0,p.action==='sleep'?.35:1.08,0])).project(activeCamera);const visible=family.shown&&!state.loading&&$('family-dialogue').checked&&p.speech?.until>family.time&&(state.floor==='ALL'||(!p.transit&&state.floor===p.floor));bubble.hidden=!visible||point.z>1||point.z< -1||Math.abs(point.x)>.95||Math.abs(point.y)>.95;if(!bubble.hidden){bubble.textContent=p.name+'：'+p.speech.text;bubble.style.left=(point.x*.5+.5)*w+'px';bubble.style.top=(-point.y*.5+.5)*h+'px';}}
}
...
function deviceChanged(change){
 if(change.type==='door'&&!family?.adjustingDoor){const door=devices.doors.get(change.id);if(door)door.userOperated=true;}
...
try{let navigation,layout;[manifest,rooms,overviewDefinition,navigation,layout]=await Promise.all([json('model/manifest.json?v=interactive-2'),json('rooms.json'),json('model/overview.json?v=landscape-1'),json('model/resident-navigation.json?v=1'),json('model/family-layout.json?v=1')]);devices=new HouseInteractions(manifest.interactions,deviceChanged,reduced);family=new FamilyLife(navigation,devices,layout,{paused:reduced});initRenderer();await changeFloor('ALL');}catch(e){failure(e);}
