...
function updateMode(){if(!renderer||!rooms)return;if(state.floor==='ALL'&&state.mode==='plan'){changeFloor('F1');return;}document.querySelectorAll('[data-mode]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.mode===state.mode)));const render=state.mode==='render';stage.hidden=render;$('render-view').hidden=!render;if(tour)ui.tourState(tour);if(render)return;activeCamera=state.mode==='plan'?ortho:camera;controls.object=activeCamera;controls.enableRotate=state.mode==='3d';if(state.mode==='plan'){state.inside=false;state.cut=true;pressed(['cut','solid'],'cut');const target=vec(floorInfo[state.floor].center);ortho.position.copy(target.clone().add(vec([0,40,.0001])));ortho.up.set(0,0,-1);controls.target.copy(target);ortho.lookAt(target);setOrtho();applyCut();$('view-name').textContent='本层全貌 · 正上方布局';}else if(!state.inside){if(state.room)roomView(false);else resetView();}resize();}
...
function updateFamilyUi(){
 ...
 if(refresh){ui.tourState(tour);document.getElementById('family-state').textContent=tour.active?'带看期间暂停':family.paused?'已暂停':family.shown?'日常进行中':'已隐藏';$('tv-toggle').textContent=appliances.tvOn?'电视已开 · 关闭':'电视已关 · 打开';$('tv-toggle').setAttribute('aria-pressed',String(appliances.tvOn));}
}
