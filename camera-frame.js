...
export function personViewEye(root, target, cutaway) {
  ...
  for (const offset of offsets) {
    const eye = target.clone().add(new THREE.Vector3(...offset));cutaway.update(eye, target);
    let score = 0;
    for (const height of [-.25, .2]) {
      ...
      for (const hit of ray.intersectObject(root, true)) {
        ...
        if (visible && material?.colorWrite !== false && material?.opacity > .5 && !cutaway.transparency.isFaded(mesh, hit.point, hit.faceIndex)) blockers.add(mesh.uuid);
      }
      score += blockers.size;
    }
